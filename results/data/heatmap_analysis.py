"""
Detailed heatmap analysis for k2_vs_k1 grid results.

This script upgrades the previous brief dump and produces a thorough,
human-readable report and annotated figures for each saved NPZ generated by
`src/experiments/k_comparison_heatmaps.py`.

What it does:
 - Loads NPZ files (default: A, B, C in results/data)
 - Prints a clear explanation of what arrays are present and what they mean
   (names, dtypes, shapes)
 - Computes summary statistics for numeric arrays (min/mean/max/std)
 - When `prob_rewards`/`prob_hints` are present, maps rows/cols to these axes
 - Renders annotated heatmaps (with tick labels from the saved grids)
 - Writes a Markdown summary next to the NPZ and saves PNGs to results/figures

Usage:
	python results/data/heatmap_analysis.py            # analyze A, B, C
	python results/data/heatmap_analysis.py -f A -o report.md  # single file

This script is safe to run from the project root.
"""
